...
def display():
    ...
    with col2:
        ...
        """)
        output_dict = {}                    
        #output_dict["output_directory"] = st.text_input("Enter output directory:", value="")
        output_dict["export_name"] = st.text_input("Enter filename for export:", value="model_output.pkl")

        #if os.path.isdir(output_dict["output_directory"]) and 
        if output_dict["export_name"].endswith('.pkl'):
            EXPORT_FLAG = True
        else:
            st.error("Error: Please enter an existing directory and \
        ...
        #Import results
        st.header("Import Results", divider="gray")
        st.write("""\
